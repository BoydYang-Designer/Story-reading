...
def download_youtube_video(url, save_path, quality='best'):
    """
    使用 yt-dlp 下載 YouTube 影片
    quality: 'best' (最高畫質) 或 '1080p' / '720p' / '480p' 等
    """
    if not YTDLP_AVAILABLE:
        print("  ✗ 尚未安裝 yt-dlp，請執行: pip install yt-dlp")
        return False
    ...
    def progress_hook(d):
        if d.get('status') == 'downloading':
            ...
            print("\r下載進度: 100.0%  轉檔中...")

    ydl_opts = {
        'format': format_selector,
        'outtmpl': os.path.join(save_path, '%(title).100s.%(ext)s'),
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # merge_output_format 可能會把副檔名換成 mp4
            base, _ = os.path.splitext(filename)
            final_path = base + '.mp4'
            if os.path.exists(final_path):
                filename = final_path

        print(f"\n✓ 影片已儲存: {os.path.basename(filename)}")
...
def download_video(url, save_path):
    ...
    if platform == 'tiktok':
        return download_tiktok_video(url, save_path)
    elif platform == 'youtube':
        return download_youtube_video(url, save_path)
    else:
        print("  ✗ 無法辨識的連結（僅支援 TikTok 與 YouTube）")
        return False
...
def main():
    ...
    print("=" * 60)
    print("影片下載器 v3.0")
    print("支援 TikTok（多 API 自動切換）與 YouTube（yt-dlp）")
    print("=" * 60)

    if not YTDLP_AVAILABLE:
        print("提醒：尚未安裝 yt-dlp，YouTube 連結將無法下載。")
        print("請執行: pip install yt-dlp\n")
    ...
    if batch_mode:
        print("\n--- 批次下載模式 ---")
        print("請輸入 TikTok 或 YouTube 影片連結（每次輸入一個）")
        print("輸入空白或按取消結束\n")
        
        while True:
            url = simpledialog.askstring(
                "輸入影片連結",
                "請貼上 TikTok 或 YouTube 影片連結:\n（按取消或留空結束）"
            )
