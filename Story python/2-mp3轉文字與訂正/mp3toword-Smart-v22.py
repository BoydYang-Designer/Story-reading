...
def show_diff_dialog(filename, old_text, new_text):
    ...
    win = tk.Toplevel()
    win.title(f"🔧 斷點編輯模式 - {filename}  （重新檢查）")
    ...
    hdr.pack(fill=tk.X)
    tk.Label(hdr, text=f"🔧 斷點編輯模式 - {filename}",
             font=('Microsoft JhengHei', 13, 'bold'),
             fg='white', bg='#1a252f').pack()
    tk.Label(hdr, text='即使內容完全相同，您仍可拖曳詞語、拆分、合併、恢復原始斷點',
             font=('Microsoft JhengHei', 10), fg='#58a6ff', bg='#1a252f').pack()
    ...
    only_resplit  = (' '.join(old_words_all) == ' '.join(new_words_all))

    stat = tk.Frame(win, bg='#ecf0f1', pady=5)
    stat.pack(fill=tk.X, padx=10, pady=(5,0))
    status_txt = ('✅ 文字內容完全相同，您仍可進入編輯器微調斷點' if only_resplit
        ...
        if save_timestamp:
            ...
            try:
                ...
                if auto_mode and os.path.exists(ts_path):
                    ...
                    print(f"  [重新檢查] 強制進入斷點編輯模式（可拖曳/拆分/合併）: {os.path.basename(ts_path)}")

                    choice, custom_segs = show_diff_dialog(
                    ...
                    if choice == 'new':
                        ...
                        print(f"  [自訂] 已儲存手動調整版本（{len(custom_segs)} 條）: {os.path.basename(ts_path)}")

                    else:
                        print(f"  [略過] 使用者選擇略過此檔: {os.path.basename(ts_path)}")
                else:
                    with open(ts_path, "w", encoding="utf-8") as f:
                        f.write(new_ts_content)
