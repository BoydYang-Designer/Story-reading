...
def show_edit4_dialog(filename, initial_lines):
    ...
    tk.Label(hdr, text=f"✂  {filename}",
             ...
             fg='white', bg='#1a252f').pack()
    tk.Label(hdr,
             text='游標定位後按 Enter 切斷  ／  行首按 Backspace 合併上一行  ／  行尾按 Delete 合併下一行',
...
if mode_choice == "2":
    ...
    recheck_mode   = True   # 讓篩選步驟也納入有舊檔的檔案
    edit_only_mode = True
...
"""
    ...
    mode_lbl = "全自動模式（自動備份+覆蓋）" if auto_mode else \
               "純編輯模式（轉錄後進入純文字斷句編輯）" if edit_only_mode else \
