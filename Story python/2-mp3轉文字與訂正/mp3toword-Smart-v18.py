...
def show_diff_dialog(filename, old_text, new_text):
    ...
    # ════════════════════════════════════════════════════════════
    # Tab 3：斷點編輯器（僅時間戳記檔可用）
    # ════════════════════════════════════════════════════════════
    tab_edit = tk.Frame(nb, bg='#0d1117')
    nb.add(tab_edit, text='  ✂ 斷點編輯  ')

    # ── 編輯器狀態 ──
    # 每條 seg 結構：{'start', 'end', 'text', 'origins': [{'start','end','text'}, ...]}
    # origins 記錄這條是由哪些原始段落合併來的，合併時串接，拆分時清空（變成各自單一 origin）
    # 若 len(origins) > 1 → 顯示「↩ 恢復原始斷點」按鈕
    def make_seg(start, end, text, origins=None):
        o = origins if origins is not None else [{'start': start, 'end': end, 'text': text}]
        return {'start': start, 'end': end, 'text': text, 'origins': o}
    ...
    edit_canvas.bind_all('<MouseWheel>', lambda e: edit_canvas.yview_scroll(int(-1*(e.delta/120)), 'units'))
    ...
    def rebuild_edit_ui():
        ...
        for idx, seg in enumerate(segs):
            ...
            is_merged  = len(origins) > 1  # 曾被合併過 → 可恢復

            # ── 句子卡片背景：合併過的用偏暖底色提示 ──
            card_bg = '#1e1a2e' if is_merged else '#161b22'
            ...
            ts_txt = f"  [{fmt_time(seg['start'])} → {fmt_time(seg['end'])}]"
            tk.Label(hdr_row, text=ts_txt, bg=card_bg, fg='#58a6ff',
                     font=('Consolas', 9), anchor='w').pack(side=tk.LEFT)
            if is_merged:
                tk.Label(hdr_row, text=f' ⊕ 已合併 {len(origins)} 條 ',
                         bg='#3a2060', fg='#c792ea',
                         font=('Microsoft JhengHei', 8, 'bold'),
                         padx=6, pady=1).pack(side=tk.LEFT, padx=6)

            # 文字顯示（每個詞可點選插入斷點）
            txt_frame = tk.Frame(card, bg='#1c2129', padx=6, pady=4)
            txt_frame.pack(fill=tk.X, pady=(4, 0))

            words = seg['text'].split()
            for wi, word in enumerate(words):
                w_lbl = tk.Label(txt_frame, text=word + ' ', bg='#1c2129', fg='#e6edf3',
                                 font=('Microsoft JhengHei', 11), cursor='hand2',
                                 padx=1, pady=1)
                w_lbl.pack(side=tk.LEFT)
                def on_enter(e, lbl=w_lbl): lbl.config(bg='#2d4a2d', fg='#56d364')
                def on_leave(e, lbl=w_lbl): lbl.config(bg='#1c2129', fg='#e6edf3')
                w_lbl.bind('<Enter>', on_enter)
                w_lbl.bind('<Leave>', on_leave)
                if wi < len(words) - 1:
                    w_lbl.bind('<Button-1>', lambda e, si=idx, wi_=wi: do_split(si, wi_))
            ...
            if len(words) > 1:
                tk.Label(action_row, text='↑ 點擊詞語可插入新斷點',
                         bg=card_bg, fg='#484f58',
                         font=('Microsoft JhengHei', 8)).pack(side=tk.LEFT)

            if is_merged:
                # 顯示各原始段落的預覽（最多 3 條，其餘省略）
                preview_texts = [o['text'][:30] + ('…' if len(o['text']) > 30 else '')
                                 for o in origins[:3]]
                preview_str = '　/　'.join(f'「{t}」' for t in preview_texts)
                if len(origins) > 3:
                    preview_str += f'　… 共 {len(origins)} 條'

                restore_fr = tk.Frame(card, bg='#2a1e40', padx=6, pady=4)
                restore_fr.pack(fill=tk.X, pady=(4, 0))
                tk.Label(restore_fr,
                         text=f'原始斷點預覽：{preview_str}',
                         bg='#2a1e40', fg='#9e7ac7',
                         font=('Microsoft JhengHei', 8),
                         wraplength=1000, justify=tk.LEFT).pack(side=tk.LEFT, fill=tk.X, expand=True)
                tk.Button(restore_fr,
                          text='↩ 恢復原始斷點',
            ...
            # ── 卡片間：合併按鈕（最後一條不顯示）──
            if idx < len(segs) - 1:
                merge_row = tk.Frame(edit_inner, bg='#0d1117', pady=2)
                merge_row.pack(fill=tk.X, padx=10)
                tk.Frame(merge_row, bg='#30363d', height=1).pack(
                    side=tk.LEFT, fill=tk.X, expand=True, pady=8)
                tk.Button(merge_row, text='🔗 合併這兩條',
        ...
        # 段落總數
        count_fr = tk.Frame(edit_inner, bg='#161b22', pady=4)
        count_fr.pack(fill=tk.X, padx=10, pady=(8, 4))
        tk.Label(count_fr, text=f'  📊 目前共 {len(segs)} 條段落',
                 bg='#161b22', fg='#8b949e',
                 font=('Microsoft JhengHei', 9)).pack(anchor='w')

    def do_merge(idx):
        """合併 idx 和 idx+1，origins 串接以便日後恢復。"""
        segs = edit_state['segs']
        if idx + 1 >= len(segs):
            return
        a, b = segs[idx], segs[idx + 1]
        merged_origins = a.get('origins', [a]) + b.get('origins', [b])
        merged = make_seg(
            start   = a['start'],
            end     = b['end'],
    ...
    def do_restore(seg_idx):
        """把已合併的段落恢復成原始多條段落（含原始時間戳記）。"""
        segs = edit_state['segs']
        seg = segs[seg_idx]
        origins = seg.get('origins', [])
        if len(origins) <= 1:
            return  # 沒有可恢復的原始斷點
        # 把 origins 各自轉回 make_seg（每條 origin 的 origins 只有自己）
    ...
    def do_split(seg_idx, word_idx):
        """在 word_idx 之後插入新斷點，時間按字數比例估算。
        拆分後兩條都只有自己作為 origin（不再帶有父層的 origins）。
        """
        segs = edit_state['segs']
        seg  = segs[seg_idx]
        words = seg['text'].split()
