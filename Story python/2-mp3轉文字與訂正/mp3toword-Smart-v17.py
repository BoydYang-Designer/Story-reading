...
def show_diff_dialog(filename, old_text, new_text):
    ...
    # ── 編輯器狀態 ──
    # edited_segs: list of {'start', 'end', 'text'}
    # 初始化：用新版的段落，若無時間戳記段落則用舊版
    if new_segs:
        edited_segs = [{'start': s, 'end': e, 'text': t} for s, e, t in new_segs]
    elif old_segs:
        edited_segs = [{'start': s, 'end': e, 'text': t} for s, e, t in old_segs]
    else:
        edited_segs = []
    ...
    tk.Label(info_fr, text='✂ 斷點編輯器：點擊句子間的按鈕來合併或拆分，完成後按底部「使用自訂版本」儲存',
             font=('Microsoft JhengHei', 10), bg='#161b22', fg='#c9d1d9', padx=12).pack(anchor='w')
    tk.Label(info_fr, text='💡 合併：把兩條句子合成一條（時間戳記自動延伸）｜拆分：在選定詞語後斷開成兩條',
    ...
    def rebuild_edit_ui():
        ...
        for idx, seg in enumerate(segs):
            # ── 句子卡片 ──
            card = tk.Frame(edit_inner, bg='#161b22', bd=1, relief=tk.SOLID, padx=8, pady=6)
            card.pack(fill=tk.X, padx=10, pady=(6, 0))

            # 時間戳記標籤
            ts_txt = f"  [{fmt_time(seg['start'])} → {fmt_time(seg['end'])}]"
            tk.Label(card, text=ts_txt, bg='#161b22', fg='#58a6ff',
                     font=('Consolas', 9), anchor='w').pack(fill=tk.X)

            # 文字顯示（可點選以觸發拆分）
            txt_frame = tk.Frame(card, bg='#1c2129', padx=6, pady=4)
            txt_frame.pack(fill=tk.X, pady=(2, 0))

            words = seg['text'].split()
            for wi, word in enumerate(words):
                # 每個詞都是可點選的 Label，點擊後在該詞「之後」拆分
                def make_split_cmd(seg_idx, word_idx):
                    def cmd():
                        do_split(seg_idx, word_idx)
                    return cmd
            ...
            if idx < len(segs) - 1:
                ...
                tk.Button(merge_row, text='🔗 合併這兩條',
                          ...
                          command=make_merge_cmd(idx)).pack(side=tk.LEFT, padx=8)
    ...
    def do_merge(idx):
        """合併 idx 和 idx+1 兩條段落。時間戳記：start=idx.start, end=idx+1.end"""
        segs = edit_state['segs']
        if idx + 1 >= len(segs):
            return
    ...
    def do_split(seg_idx, word_idx):
        """在 seg_idx 的 word_idx 之後拆分。
        時間戳記按字數比例分配（無 word-level timing 時的最佳近似）。
        """
        segs = edit_state['segs']
        seg = segs[seg_idx]
        words = seg['text'].split()
        ...
        mid_time = seg['start'] + (seg['end'] - seg['start']) * ratio

        seg_a = {'start': seg['start'], 'end': mid_time, 'text': text_a}
        seg_b = {'start': mid_time,     'end': seg['end'],   'text': text_b}
