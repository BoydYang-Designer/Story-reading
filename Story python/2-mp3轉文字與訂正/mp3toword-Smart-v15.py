...
def smart_sentence_split(segments, max_gap=0.5, max_duration=5.0, max_words=15, min_words=4):
    ...
    【待斷模式（超過軟上限後）】
      詞數 ≥ max_words 或時長 ≥ max_duration → 進入待斷模式。
      在待斷模式下找「語意安全的斷點」：
        ✅ 安全：逗號後下一詞是 and/but/or/so/yet/nor（並列）
        ✅ 安全：逗號後下一詞是大寫（新主語開始）
        ✅ 安全：時間間隔 > max_gap（說話者自然停頓）
        ❌ 不安全：逗號後下一詞是從屬連接詞（that/for/which/who/
                   because/when/where/as/though...）→ 跳過，繼續等

    【連接詞保護（正常模式下）】
      尚未進入待斷、詞數少時，時間間隔斷句不在連接詞後執行。

    【殘句合併】
      詞數 < min_words 或以接續介詞開頭 → 併入前一條。
    ═══════════════════════════════════════════════════════
    """
    SE  = {'.', '!', '?', '。', '！', '？'}   # 句尾（必斷）
    PP  = {',', ';', ':', '，', '；', '：'}   # 停頓（條件斷）

    # 從屬連接詞：接這些詞的子句依賴上文，斷在這裡下句語意不完整
    SUBORD = {
    ...
    def do_flush():
        ...
        if not txt:
            return
        ws = txt.split()
        fw_raw   = ws[0] if ws else ''               # 保留原始大小寫
        fw_clean = fw_raw.lower().rstrip('.,!?;:')   # 小寫版供比對
        # 殘句條件：
        #   1. 詞數太少
        #   2. 小寫開頭（不是句首）且是接續介詞 → 才是殘句
        #      大寫開頭（如 "In another moment..."）即使是介詞也是新句
        is_fragment = (
            len(ws) < min_words or
    ...
    def next_word_clean(future_words, pos):
        """取 pos 之後第一個非空詞（已去除標點），用於語意判斷。"""
        for wi in future_words[pos:]:
            nw = (wi.get('word','') if isinstance(wi,dict) else wi.word).strip()
            if nw:
                return nw.lower().lstrip().rstrip('.,!?;:')
        return ''

    def is_safe_break(next_wclean):
        """
        判斷在當前逗號後斷句，下句開頭是否語意完整。
        - 並列連接詞（and/but...）：安全，下句可獨立
        - 大寫開頭詞（新主語）：安全
        - 從屬連接詞（that/for/which...）：不安全，下句依賴上文
        - 空字串（末尾）：安全（讓其他規則處理）
        """
        if not next_wclean:
            return True
        if next_wclean in COORD:
            return True
        if next_wclean in SUBORD:
            return False
        # 原詞大寫開頭（非連接詞）→ 安全
        return True
    ...
    for segment in segments:
        ...
        for idx, wi in enumerate(words):
            ...
            # 下一個詞（預讀，用於語意判斷）
            nw = next_word_clean(words, idx + 1)
            ...
            # ══ 規則 B：待斷模式下找語意安全斷點 ══
            if st['waiting']:
                # 極長緊急模式（詞數 ≥ 25）：從屬連接詞保護也解除，任何逗號都可斷
                emergency = st['wc'] >= 25
                # B1：逗號/停頓 → 檢查下句開頭是否安全（緊急時跳過語意檢查）
                if ep and (emergency or is_safe_break(nw)):
                    st['e'] = we; st['t'] += ' ' + w; st['lw'] = wclean
                    do_flush(); continue
                # B2：時間間隔（說話者停頓）→ 安全，立刻斷
                if st['t'] and (ws - st['e']) > max_gap:
                    st['e'] = we; st['t'] += ' ' + w; st['lw'] = wclean
                    do_flush(); continue

            # ══ 規則 C：正常模式，時間間隔斷句（連接詞保護） ══
            if st['t'] and (ws - st['e']) > max_gap:
                # 連接詞保護：詞數少且上一詞是連接詞時不斷
