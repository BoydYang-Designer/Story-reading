...
def _make_parts(sent: str, target_alpha_idx: int, tag: str) -> list:
    """
    把句子拆成 (token, tag) 列表，對第 target_alpha_idx 個「字詞」套用 tag。
    字詞定義與 re.findall(r"[A-Za-z']+") 一致（含縮寫如 It's）。
    """
    parts   = []
    # 用與 tgt_rw / _find_alpha_idx 相同的 pattern 切分，才能對齊 index
    ...
    for op, i1, i2, j1, j2 in sm.get_opcodes():
        ...
        for k, (tw, rw) in enumerate(zip(tc, rc)):
            ...
            if hint_idx >= len(tgt_rw):
                continue
            # 用 _find_alpha_idx 精確定位 TXT 中的詞位置（避免 normalize 後的 index 偏移）
...
class ReviewWindow(tk.Toplevel):
    def __init__(self, parent, all_candidates: dict, use_ai: bool):
        ...
        self.check_vars  = {}
        self.row_frames  = {}
        self.edit_vars   = {}   # key -> StringVar，存使用者手動編輯的修改詞
