...
修復紀錄（v1.4）：
  - [行為調整] 網路錯誤不再等同「額度用完」。新增獨立的 NetworkError，
               單句翻譯失敗時最多重試 3 次（間隔 2 秒），仍失敗才跳過該句
               （保留原文英文行，不中斷整批翻譯），繼續翻譯後面的句子。
  - [行為調整] 只有「額度真的用完」（MyMemory WARNING 或翻譯結果不含中文）
               才會存檔並中斷整批翻譯；其餘情況（單一檔案翻完、單句網路
               重試後跳過）都會自動接著處理下一句 / 下一個檔案，直到全部
               檔案完成或真正額度用完才停止。
"""
...
# [v1.4] 網路錯誤重試設定：網路問題不再等同「額度用完」，
# 會先重試幾次，仍失敗才跳過該句繼續下一句，不會中斷整批翻譯。
...
class NetworkError(Exception):
    """[v1.4] 網路 / 連線層級的暫時性錯誤，與『額度用完』分開處理。
    只會導致單一句子重試或跳過，不會中斷整批翻譯。"""
    pass
...
def translate_text(text: str, email: str = "") -> str:
    """
    呼叫 MyMemory API 翻譯。
    - 額度用完（MYMEMORY WARNING / 結果不含中文）→ 拋出 QuotaExceededError，
      會中斷整批翻譯（存檔並停止）。
    - 網路 / 連線 / JSON 解析錯誤 → 拋出 NetworkError，
      屬於暫時性問題，由呼叫端（process_file）重試，重試仍失敗才跳過該句，
      不會中斷整批翻譯。
    ...
    [Bug Fix v1.4]
      原本網路錯誤會被視為 QuotaExceededError，導致單純斷線也會讓整批
      翻譯直接停止。現在改為獨立的 NetworkError，交給 process_file 做
      「重試 → 仍失敗則跳過該句、繼續下一句」，只有真正額度用完才停止。
    """
    text = text.strip()
    if not text:
        return ""
    ...
    try:
        ...
        # [v1.4] 網路錯誤、JSON 解析失敗：視為暫時性問題，
        # 交給 process_file 重試，不再直接等同額度用完
        raise NetworkError(f"網路錯誤：{net_err}") from net_err
...
def process_file(filepath: str, email: str,
    ...
    while i < len(raw_lines):
        ...
        # 需要翻譯
        # [v1.4] 網路錯誤：重試 NETWORK_MAX_RETRIES 次，仍失敗才跳過本句、
        #        繼續下一句（不中斷整批翻譯）。
        # 額度用完（QuotaExceededError）：立即存檔並中斷整批翻譯，行為不變。
        for attempt in range(1, NETWORK_MAX_RETRIES + 1):
            try:
                ...
                if attempt < NETWORK_MAX_RETRIES:
                    log_fn(f"  🔁 網路錯誤（第 {attempt}/{NETWORK_MAX_RETRIES} 次重試）：{e}")
                    time.sleep(NETWORK_RETRY_DELAY)
                else:
                    log_fn(f"  ⏭️  網路錯誤，重試 {NETWORK_MAX_RETRIES} 次後仍失敗，"
                           f"跳過此句（保留原文，下次會自動重翻）：{sentence[:45]}")

        # 網路錯誤跳過時：原始英文行已在前面 append，不補中文行，
        # 下次重新開啟同一檔案時，此句仍會被判定為「需要翻譯」而自動重試。
